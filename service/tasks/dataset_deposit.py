...
DEPOSIT_ERROR_MSG = """
There was an error attempting to deposit a dataset with job_id {x}.  The message from the SWORDv2 deposit
code was:
...
def process_datasets():
    ...
    deposit_queue = app.config.get('DATASET_SUBMIT_QUEUE')
...
def _do_deposit(r, job_id):
    ...
    try:
        ...
        if sword_deposit.deposit_dataset():
...
def _record_deposit_error(r, job_id, msg):
    ...
    to = app.config.get("DEPOSIT_ERROR_EMAIL")
    if to:
        mail.send_mail([to], "Dataset Deposit Error", msg_body=DEPOSIT_ERROR_MSG.format(x=job_id, y=msg))

def _send_confirmation(job_id):
    record = models.Dataset.pull(job_id)
    ...
    subject = app.config.get("DATASET_CONFIRM_SUBJECT", "")
    body_template = None
    if record.is_placeholder():
        body_template = app.config.get("DATASET_CONFIRM_BODY_PLACEHOLDER", "")
    else:
        body_template = app.config.get("DATASET_CONFIRM_BODY_FINAL", "")
