from service.dao import EthesisDAO
...
class Ethesis(dataobj.DataObj, EthesisDAO):
    ...
    @property
    def dir(self):
        job_id = self.ethesis_id
        file_dir = os.path.join(app.config.get('ETHESIS_UPLOAD_DIR'), job_id[:2], job_id[2:4], job_id[4:])
        return file_dir
    ...
    def burn_dir(self):
        ...
        parent1 = os.path.join(app.config.get('ETHESIS_UPLOAD_DIR'), job_id[:2], job_id[2:4])
        if os.path.exists(parent1) and len(os.listdir(parent1)) == 0:
            shutil.rmtree(parent1)

        parent2 = os.path.join(app.config.get('ETHESIS_UPLOAD_DIR'), job_id[:2])
    ...
    @property
    def crsid(self):
        authors = self._get_list("authors")
        for a in authors:
            return a.get("crsid")
